"""
Modul de licențiere pentru integrarea Pago Plătește.
...
STORAGE_KEY = "pagoplateste_license"
STORAGE_VERSION = 1

# Salt intern pentru fingerprint (unic per integrare)
_FP_SALT = "pAg0_pL@t3$t3_Ha$h_2026!rX"
...
class LicenseManager:
    """Gestionează licența pentru integrarea Pago Plătește.
    ...
    async def async_load(self) -> None:
        ...
        _LOGGER.debug("[Pago:License] Încep async_load()")
        try:
            stored = await self._store.async_load()
            self._data = dict(stored) if stored else {}
        except Exception:  # noqa: BLE001
            _LOGGER.warning(
                "[Pago:License] Storage corupt sau ilizibil "
                "— pornesc cu date goale (serverul va restaura starea)"
            )
            self._data = {}
        _LOGGER.debug(
            "[Pago:License] Date din storage: %d chei (%s)",
            len(self._data),
        ...
        _LOGGER.debug(
            "[Pago:License] Fingerprint generat: %s... (hw: %s...)",
        ...
        if self._status_token:
            ...
            _LOGGER.debug(
                "[Pago:License] Cache restaurat: status=%s, cache_valid=%s",
                cached_status,
                cache_valid,
            )
        else:
            _LOGGER.debug("[Pago:License] Niciun cache de status — prima rulare")

        # Verifică statusul la server (prima verificare la startup)
        _LOGGER.debug("[Pago:License] Verific statusul la server (startup)...")
        await self.async_check_status()

        self._loaded = True
        final_status = self.status
        _LOGGER.debug(
            "[Pago:License] async_load() finalizat — status=%s, is_valid=%s",
            final_status,
            self.is_valid,
        )
        ...
        if final_status == "licensed":
            ...
            _LOGGER.info(
                "[Pago:License] ✓ Licență ACTIVĂ (cheie: %s)", key
            )
        elif final_status == "trial":
            days = self.trial_days_remaining
            _LOGGER.info(
                "[Pago:License] ⏳ Perioadă de evaluare (trial): "
                "%d zile rămase",
                days,
            )
        elif final_status == "expired":
            _LOGGER.warning(
                "[Pago:License] ✗ EXPIRAT — perioada de evaluare "
                "sau licența a expirat. Senzorii nu vor funcționa."
            )
        else:
            _LOGGER.warning(
                "[Pago:License] ✗ FĂRĂ LICENȚĂ (status=%s) — "
    ...
    async def _async_save(self) -> None:
        ...
        _LOGGER.debug("[Pago:License] Salvez datele în storage")
    ...
    async def async_check_status(self) -> dict[str, Any]:
        ...
        if self._is_status_cache_valid():
            _LOGGER.debug(
                "[Pago:License] Cache valid — folosesc token existent "
        ...
        _LOGGER.debug(
            "[Pago:License] Cache expirat sau inexistent — "
        ...
        try:
            ...
            async with session.post(
                ...
                _LOGGER.debug(
                    "[Pago:License] Server /check răspuns: HTTP %d",
                    resp.status,
                )
                ...
                if resp.status == 200 and "status" in result:
                    ...
                    if not self._verify_token_signature(result):
                        _LOGGER.warning(
                            "[Pago:License] Semnătura token-ului de status "
                    ...
                    if server_key and self._data.get("license_key") != server_key:
                        ...
                        _LOGGER.debug(
                            "[Pago:License] license_key sincronizat "
                    ...
                    _LOGGER.debug(
                        "[Pago:License] Status actualizat de la server — %s "
                        "(valid_until: %s)",
                        server_status,
                    ...
                    if server_status == "expired":
                        _LOGGER.warning(
                            "[Pago:License] Server confirmă: EXPIRAT "
                            "(trial_days_remaining=0)"
                        )
                    elif server_status == "trial":
                        _LOGGER.info(
                            "[Pago:License] Server confirmă: TRIAL "
                            "(zile rămase: %s)",
                ...
                if result.get("error") == "invalid_hmac":
                    if self._data.get("client_secret") and not self._hmac_retry_done:
                        _LOGGER.warning(
                            "[Pago:License] HMAC invalid — client_secret "
                    ...
                    _LOGGER.error(
                        "[Pago:License] HMAC invalid (retry epuizat). "
                        "Serverul nu recunoaște acest dispozitiv."
                    )
                else:
                    _LOGGER.warning(
                        "[Pago:License] răspuns invalid de la /check — %s",
                        result,
                    )
                return self._status_token
            ...
            _LOGGER.warning(
                "[Pago:License] eroare de rețea la verificare status "
                "(încercare #%d) — %s",
            ...
            self._consecutive_failures += 1
            _LOGGER.warning(
                "[Pago:License] eroare neașteptată la verificare status "
                "(încercare #%d) — %s",
                self._consecutive_failures,
                err,
            )
    ...
    def is_licensed(self) -> bool:
        ...
        if not self._verify_token_signature(token):
            _LOGGER.warning("[Pago:License] semnătură token activare invalidă")
            return False

        if token.get("fingerprint") != self._fingerprint:
            _LOGGER.warning("[Pago:License] fingerprint token nu se potrivește")
            return False

        expires_at = token.get("expires_at")
        if expires_at and time.time() > expires_at:
            _LOGGER.info("[Pago:License] licența a expirat (token local)")
            return False
        ...
        if self._status_token and self._is_status_cache_valid():
            ...
            if server_status not in ("licensed", "trial"):
                _LOGGER.warning(
                    "[Pago:License] serverul raportează status '%s' — licență invalidă",
                    server_status,
                )
                return False
        ...
        if self._status_token and not self._is_status_cache_valid():
            if self._is_within_grace_period():
                if not self._cache_expiry_warned:
                    ...
                    _LOGGER.warning(
                        "[Pago:License] cache expirat — funcționare în perioadă "
                ...
                if not self._cache_expiry_warned:
                    _LOGGER.warning(
                        "[Pago:License] cache expirat + perioadă de grație depășită "
    ...
    async def async_heartbeat(self) -> bool:
        ...
        try:
            ...
            async with session.post(
                ...
                _LOGGER.warning(
                    "[Pago:License] heartbeat respins — %s",
                    result.get("error", "necunoscut"),
                )
                return False

        except Exception:  # noqa: BLE001
            _LOGGER.debug("[Pago:License] heartbeat eșuat (rețea indisponibilă)")
            return False
    ...
    async def async_activate(self, license_key: str) -> dict[str, Any]:
        ...
        try:
            ...
            async with session.post(
                ...
                _LOGGER.debug(
                    "[Pago:License] /activate răspuns: HTTP %d",
                    resp.status,
                )
                ...
                if resp.status != 200:
                    ...
                    _LOGGER.warning(
                        "[Pago:License] activare eșuată — HTTP %d: %s",
                        resp.status,
                        body[:500],
                    )
                ...
                if result.get("success"):
                    ...
                    _LOGGER.info(
                        "[Pago:License] licență activată cu succes (%s)",
                ...
                _LOGGER.warning(
                    "[Pago:License] activare eșuată — %s (răspuns: %s)",
                    error,
                    result,
                )
            ...
            _LOGGER.error(
                "[Pago:License] eroare de rețea la activare — %s", err
            )
            return {"success": False, "error": "network_error"}
        except Exception as err:  # noqa: BLE001
            _LOGGER.error(
                "[Pago:License] eroare neașteptată la activare — %s", err
            )
        ...
        try:
            ...
            async with session.post(
                ...
                if resp.status == 200 and result.get("success"):
                    ...
                    _LOGGER.info("[Pago:License] licență dezactivată cu succes")
            ...
            _LOGGER.error("[Pago:License] eroare la dezactivare — %s", err)
            return {"success": False, "error": "network_error"}
            ...
            async with session.post(
                ...
                if resp.status == 200:
                    ...
                    if not result.get("success"):
                        _LOGGER.warning(
                            "[Pago:License] Server a refuzat '%s': %s",
                            action,
                            result.get("error"),
                        )
                else:
                    _LOGGER.warning(
                        "[Pago:License] Notify HTTP %d pentru '%s'",
                        resp.status,
                        action,
                    )
        except Exception as err:  # noqa: BLE001
            _LOGGER.debug(
                "[Pago:License] Nu s-a putut raporta '%s': %s",
                action,
                err,
            )
    ...
    async def _async_reload_entries(self) -> None:
        ...
        _LOGGER.info(
            "[Pago:License] Reîncarc %d entry-uri după schimbarea licenței",
            len(entries),
        )
    ...
    def _verify_token_signature(self, token: dict[str, Any]) -> bool:
        ...
        try:
            ...
            _LOGGER.warning(
                "[Pago:License] nicio cheie publică nu a validat semnătura"
            )
            return False

        except Exception as err:  # noqa: BLE001
            _LOGGER.debug(
                "[Pago:License] verificare semnătură eșuată — %s", err
            )
            return False
